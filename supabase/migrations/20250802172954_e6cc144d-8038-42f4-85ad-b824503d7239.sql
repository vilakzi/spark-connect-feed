-- COMPREHENSIVE SUPABASE DATABASE CLEANUP
...
-- Phase 5: Optimize active tables with proper indexes
...
CREATE INDEX IF NOT EXISTS idx_profiles_last_active ON public.profiles(last_active) WHERE last_active IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_admin_content_status ON public.admin_content(status) WHERE status IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_admin_content_category ON public.admin_content(category) WHERE category IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_posts_provider_id ON public.posts(provider_id);
CREATE INDEX IF NOT EXISTS idx_posts_expires_at ON public.posts(expires_at) WHERE expires_at > now();
