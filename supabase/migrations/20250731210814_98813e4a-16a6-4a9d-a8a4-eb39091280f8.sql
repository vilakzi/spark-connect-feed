-- Create profile views tracking (simplified)
...
CREATE POLICY "Users can view their own profile views" ON public.profile_views FOR SELECT USING (auth.uid() = viewed_id);
CREATE POLICY "Users can create profile views" ON public.profile_views FOR INSERT WITH CHECK (auth.uid() = viewer_id AND viewer_id != viewed_id);

CREATE POLICY "Users can manage their own stories" ON public.stories FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "Users can view active stories" ON public.stories FOR SELECT USING (expires_at > now() AND auth.uid() IS NOT NULL);
...
CREATE POLICY "System can log user behavior" ON public.user_behavior_analytics FOR INSERT WITH CHECK (auth.uid() = user_id);
